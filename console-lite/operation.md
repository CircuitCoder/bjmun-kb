...
点击添加可以在列表末尾添加一个席位。点击席位名可以编辑该位置的席位。在添加和编辑的过程中，按下 Esc 键取消，回车键确认，在添加时按下 Alt (Option) + 回车键可以继续添加下一个席位。
...
- 投票名
- 当前轮数/总轮数
- 如果使用文件二分之三多数，会显示文件二分之三字样。
- 投票状态
  - 绿色：赞成
  - 红色：反对
  - 黑色：弃权
  - 蓝色：目标票数。如果投票使用文件二分之三多数，计算目标票数会减去弃权票数，但是不会忽略未投票的席位。
- { 一个小人图标 } 参加的席位数

点击添加+按钮可以显示添加投票对话框，对话框内已经对每个选项的意义做出的说明。如下图片展示的是文件二分之三没有被选中的状态：
